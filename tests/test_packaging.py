...
`klode.lib.formats` bug) and against the version drifting out of its single source."""
import sys
import tomllib
import unittest
...
class Packaging(unittest.TestCase):
    ...
    def test_wheel_ships_every_package(self):
        ...
        discovered = sorted(str(p.parent.relative_to(REPO)).replace("/", ".")
                            for p in (REPO / "klode").rglob("__init__.py"))
        self.assertIn("klode.lib.formats", discovered)          # the package this test exists to protect
        ...
        else:                                                  # find-based discovery: a klode* include
            include = (pkgs or {}).get("find", {}).get("include", [])
            self.assertTrue(any(i in ("klode", "klode*", "klode.*") for i in include), include)
    ...
    def test_version_is_single_sourced(self):
        ...
        from klode.lib import __version__
