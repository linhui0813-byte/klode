...
MARK = "<!-- scaffold: managed by lodlib — edit Thin/Full below the marker by hand -->"
...
# lodlib string made `body_after_marker` miss a real hand-written body, and build then overwrote
...
def _norm(s: str) -> str:
    ...
    source — this is the `lib check` hotspot, run over every source (tens of MB) on every run.
...
def resolve(m: Marker, hays: tuple[str, str]) -> Resolution:
    ...
    if m.regex:
        ...
        # backtracking pattern can only hang the author's own `lib check`, never a third party.
