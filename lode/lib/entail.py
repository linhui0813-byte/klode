...
`lib check` proves an anchor RESOLVES; it cannot prove the quoted span ENTAILS the card's
claim, that qualifications were preserved, or that the source is credible. This module adds
that missing check as an **opt-in, warn-only** layer (`lib check --entail`).
...
`[entail]` extra; the DEFAULT lodlib path stays zero-dependency. When the extra is absent,
`lib check --entail` degrades to a clear NOTE and changes nothing — grep still gated the build.
