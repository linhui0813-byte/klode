...
def source_sha256(path: str) -> str:
    ...
    front-matter by `lode build --stamp`; `lode check` warns when the live source no longer
...
def framework_source_map(cfg: Config) -> dict[str, str]:
    ...
    # passes `lode check` but is invisible here — its framework link and consult de-dup silently break
...
def build(cfg: Config, *, stamp: bool = False) -> dict:
    ...
    if not sources and existing:
        ...
        # next `lode check` fail [D] telling the user to run the very command that emptied it.
...
def _write_index(cfg: Config, sources: list[dict]) -> None:
    ...
    idx = ["# Library card index (the board)\n",
           ...
           "demand — never invented. Generated by lode.\n",
           f"\n**{len(sources)} sources** · "
